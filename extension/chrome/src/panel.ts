...
import { ChromeGoogleDriveAuth } from "./google-drive-auth";
...
const localRecords = new BrowserLocalRecords({
  ...
  locationLabel: "in this browser profile",
});
...
const bundleRootUrl = chrome.runtime.getURL("");
...
async function initializeProveInclusion(): Promise<void> {
  ...
  configureGoogleDrive(loaded.proveInclusion.config);
...
function configureGoogleDrive(config: RuntimeConfig): void {
  ...
  if (manifestClientId !== clientId) {
    googleDriveConnection.setUnavailable(
      "Google Drive OAuth configuration does not match the extension manifest.",
    );
    return;
  }
  void googleDriveConnection.configure(new ChromeGoogleDriveAuth());
}
...
function assertRuntimeConfig(value: unknown): asserts value is RuntimeConfig {
  if (
    ...
    || !isRecord(value.googleDrive)
    || !isNormalizedGoogleOAuthClientId(value.googleDrive.clientId)
  ) {
    throw new Error("Invalid runtime config");
  }
}

function isNormalizedGoogleOAuthClientId(value: unknown): value is string {
  if (typeof value !== "string") {
    return false;
  }
  try {
    return normalizeGoogleOAuthClientId(value) === value;
  } catch {
    return false;
  }
}
